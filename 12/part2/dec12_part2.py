...
def findStartsAndArrival():
    global currentPosition, arrival, starts
    r, c = 0, 0
    for r in range(len(lines)):
        for c in range(len(lines[0])):
            if lines[r][c] == 'S':
                lines[r] = lines[r][:c] + 'a' + lines[r][c+1:]
                starts.append(Point((r,c), 0, 'a'))
            elif lines[r][c] == 'a':
                starts.append(Point((r,c), 0, 'a'))
            if lines[r][c] == 'E':
                arrival = (r,c)
...
findStartsAndArrival()
minCostToArrival = None
for start in starts:
    visitedPoints = dict()
    ...
    if costToArrival!= None and (minCostToArrival == None or minCostToArrival > costToArrival):
                minCostToArrival = costToArrival
print(minCostToArrival)